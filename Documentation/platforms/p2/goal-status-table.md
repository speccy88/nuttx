...
| --- | --- | --- | --- |
| P2-EC32MB Rev B | `--board p2-ec32mb` | active-high LEDs P38/P39; 32 MiB PSRAM; 16 MiB flash; microSD | Available at `/dev/cu.usbserial-P97cvdxp`. The specialized flat-UP baseline is HIL-verified. An interim clean-at-build `showcase` candidate ran every enabled RAM-HIL target stage; final hash-bound PASS acceptance remains pending after the host newline-capture fix is committed. |
...
| Toolchain, build wrapper, and release provenance | **PASS for the established toolchain; IN PROGRESS for final packaging.** Builds preserve source state, toolchain lock, hashes, configuration, maps, logs, and machine-readable status. | Apps commit `f77074ac4715fad525239a464fb0d1d4a8c3b279` is pushed and pinned by `tools/p2/bootstrap-local.sh`. The final NuttX SHA is still pending because the regression-tested showcase newline-capture fix and final documentation are not committed. | Commit NuttX, regenerate the runtime toolchain lock, and make clean `p2-ec32mb:showcase` and `p2-ec:showcase` artifacts from the final commits. |
| Dual-board showcase builds | **COMPILED / STATICALLY-VERIFIED in development builds.** Both raw images fit the serial SD-writer ceiling and contain their exact board marker. | The newest P2-EC32MB development candidate was clean at build time at apps `f77074ac4715fad525239a464fb0d1d4a8c3b279` / interim NuttX `6a77317aceeaa772cf3d9636153a0817fd797773`: raw 395,484 bytes, SHA-256 `fbb4f4d7ccfab6c3f53561cf0adfcfbdebeca9a6cb252eba752731589958948f`; ELF 487,072 bytes, SHA-256 `847c6fd32983f6f506055227eda8693a0f29ee27c625cb489a91fb7c8537598a`; `source_clean=true`. The last Rev D development raw image was 373,312 bytes (`a7ce4db898576d1b0c6f612631de0d15d43276a96e1dfc6a0138add234ac7510`). Neither is the final release artifact. | Produce both profiles from the final committed NuttX tree and package only `status.json` artifacts with stable commits and `source_clean=true`. |
| EC32MB development showcase HIL | **PARTIAL / IN PROGRESS.** Every enabled target action passed in RAM, but the saved host evidence status is FAIL and must not be promoted as final PASS evidence. | At the interim hashes above, 15 stages recorded PASS. The raw log then contains walking/address/boundary/random/full-memory/concurrency/timeout/recovery/CE-timing PSRAM passes and `P2PSRAM:PASS:SEQUENCE=A55A0713`. The overall exit code 3 came only from the host command-slice dropping the marker's terminating newline; applying the current `_capture_segment` fix to the saved raw segment changes the strict parser result from incomplete to complete, and its regression test passes. No destructive storage action or persistent boot was performed. | Commit the runner fix, rebuild from the final NuttX SHA, and rerun full hash-bound HIL. Keep exact packaged flash boot and SD-only boot as separate pending gates. |
...
| GitHub publication | **IN PROGRESS.** Apps commit `f77074ac4715fad525239a464fb0d1d4a8c3b279` is pushed to `codex/p2-hil-finish-apps`; the final NuttX SHA is pending. | NuttX branch `codex/p2-hil-finish`; apps branch `codex/p2-hil-finish-apps`. | Commit and push final NuttX, push tag `p2-edge-flat-up-v0.1.0`, create the prerelease, verify remote assets/checksums, and open the draft PR. |
...
| 2 | Freeze source | Apps SHA `f77074ac4715fad525239a464fb0d1d4a8c3b279` is pinned by the bootstrap; the final NuttX SHA is recorded after the runner fix and documentation are committed, with both trees clean. |
